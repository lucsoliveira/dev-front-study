...
function generateData() {
  ...
  for (var j = 0; j < columns; j++) {
    ...
    for (var i = 0; i < rows; i++) {
      var curRow = "";
      offsetRandCharacters = getRandomInt(
        Math.round(maxRowCharacters * 0.75),
        maxRowCharacters
      );
      var breakFree = true;
...
function setupTable($t) {
  ...
  var padding =
    parseInt(th0.css("padding-left")) + parseInt(th0.css("padding-right"));
  ...
  var footerContainer = $("<table class='t_footer'></table>").insertAfter(
    tableDiv
  );
  ...
  var searchBox = $("<input type='text' placeholder='Search'/>").appendTo(
    searchPlaceholder
  );
